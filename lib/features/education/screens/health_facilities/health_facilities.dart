...
import 'package:pinkribbonbhc/features/education/screens/health_facilities/health_facilities_list.dart';
...
class HealthFacilities extends ConsumerStatefulWidget {
  const HealthFacilities({super.key});

  @override
  _HealthFacilitiesState createState() => _HealthFacilitiesState();
}

class _HealthFacilitiesState extends ConsumerState<HealthFacilities> {
...
//Markers set
  ...
  final key = 'AIzaSyAcL7c6h14swLr9yEQWg8xPqUva56jY6Tg';
...
//Initial map position on load
  ...
  _setNearMarker(LatLng point, String label, List types, String status) async {
    ...
    if (types.contains('hospital') || types.contains('doctor')) {
      markerIcon = await getBytesFromAsset(
          'assets/icons/mapicons/health-medical.png', 75);
    } else
      markerIcon =
          await getBytesFromAsset('assets/icons/mapicons/places.png', 75);
  ...
  Future<void> _requestLocationPermission() async {
    ...
    if (status.isGranted) {
      ...
      TLoaders.errorSnackBar(
          title: 'Error', message: 'Location permission denied');
      Get.back();
    }
  }
  ...
  Future<void> _getCurrentLocation() async {
    ...
    _loadNearbyPlaces(_currentLocation);
  }

  Future<void> _loadNearbyPlaces(location) async {
    if (location != null) {
      ...
      placesWithin.sort((a, b) => a['distance'].compareTo(b['distance']));
      ...
      showNearPlacesListView();
    }
  }

  Future<void> showNearPlacesListView() async {
    Positioned(
      bottom: 60.0,
      child: Container(
        height: 300.0,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _currentLocation == null
          ? Center(
              child: lottie.Lottie.asset(TImages.mapAnimation,
                  ...
                  Stack(
                    children: [
                      ...
                      searchToggle
                          ? Padding(
                              padding:
                                  EdgeInsets.fromLTRB(15.0, 40.0, 15.0, 5.0),
                              child: Column(
                                children: [
                                  Container(
                                    height: 50.0,
      ...
      floatingActionButton: FloatingActionButton(
          ...
          child: Icon(Iconsax.hospital, color: TColors.white),
          shape: CircleBorder(),
          onPressed: () {
            Get.to(() => HealthFacilitiesList());
          }),
    );
  }
  ...
  _nearbyPlacesList(index) {
    var place = allFavoritePlaces[index];
    var distanceInKm = place['distance'] != null
  ...
  Future<void> gotoSearchedPlace(double lat, double lng) async {
    ...
    _setMarker(LatLng(lat, lng));
    _loadNearbyPlaces(LatLng(lat, lng));
  }
